%\VignetteIndexEntry{MetaGxBreast: An Introduction}
%\VignetteDepends{Biobase}
%\VignetteSuggests{knitr}
%\VignetteImports{amap}
%\VignetteKeywords{Breast Cancer}
...
\title{\Rpackage{MetaGxBreast}: a package for breast cancer gene expression analysis}
