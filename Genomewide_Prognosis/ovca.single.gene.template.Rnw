...
if(survival.type == "rfs") {
  ...
  esets.with.gene <- esets.os[sapply(esets.os, function(x) entrez.ids[{{current.index}}] %in% fData(x)$EntrezGene.ID)]
}
...
survival.df.list <- lapply(esets.with.gene, function(eset) {
  ...
  # cut can replaced here with .bincode to avoid cases of non-unique quantiles!
  # Or extract only unique quantiles
  #expression.quantiles <- cut(expression.values, breaks=quantile(expression.values, probs=c(0,0.5,1)), include.lowest=TRUE)
  expression.quantiles <- cut(expression.values, breaks=unique(quantile(expression.values, probs=c(0,0.5,1))), include.lowest=TRUE)
  ...
  colnames.to.keep <- c("days_to_tumor_recurrence", "recurrence_status")
    ...
    colnames.to.keep <- c("days_to_death", "vital_status")
  }
  ...
  if(survival.type == "rfs") {
    ...
    # Rename last two columns
    colnames(survival.data)[colnames(survival.data) == "days_to_death"] <- "days_to_event"
    colnames(survival.data)[colnames(survival.data) == "vital_status"] <- "event_status"
  }
